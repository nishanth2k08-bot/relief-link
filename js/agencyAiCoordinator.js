/* ReliefLink AI Coordinator — shown only inside the Multi-Agency feed. */
(function () {
  'use strict';
  ...
  const FEED_VIEWS = new Set(['comms', 'communications', 'multi-agency', 'multiAgency', 'agencyFeed']);
  ...
  function isFeedView() {
    const view = String(getAppState().currentView || '').trim();
    return FEED_VIEWS.has(view) || /multi.?agency|agency.?feed|comms|communications/i.test(view);
  }
  ...
  function addStyles() {
    ...
    style.id = STYLE_ID;
    style.textContent = `
      #${PANEL_ID} { margin:16px 0 0; width:100%; font-family:Inter,system-ui,sans-serif; }
      #${PANEL_ID} .rl-ai-launch { border:0; border-radius:10px; padding:11px 16px; background:#2563eb; color:#fff; font-weight:800; cursor:pointer; box-shadow:0 6px 18px rgba(37,99,235,.22); display:flex; align-items:center; gap:9px; }
      #${PANEL_ID} .rl-ai-window { display:none; width:100%; max-width:720px; height:min(560px,65vh); margin-top:12px; background:#0f172a; color:#e5e7eb; border:1px solid rgba(148,163,184,.25); border-radius:16px; overflow:hidden; box-shadow:0 15px 45px rgba(0,0,0,.24); }
      #${PANEL_ID}.open .rl-ai-window { display:flex; flex-direction:column; }
      #${PANEL_ID}.open .rl-ai-launch { display:none; }
      #${PANEL_ID} .rl-ai-head { padding:14px 16px; background:#172554; border-bottom:1px solid rgba(148,163,184,.2); display:flex; align-items:center; justify-content:space-between; }
  ...
  async function sendMessage(text, ui) {
    ...
    try {
      ...
      ui.sending = false; ui.send.disabled = false; ui.input.focus();
    }
  }
  ...
  function mount() {
    if (!isFeedView() || !document.body || document.getElementById(PANEL_ID)) return;
    addStyles();
    ...
    root.innerHTML = `
      ...
      <section class="rl-ai-window" aria-label="ReliefLink AI Coordinator">
        <header class="rl-ai-head">
          <div><div class="rl-ai-title">✦ ReliefLink AI Coordinator</div><div class="rl-ai-sub">Multi-agency operational support</div></div>
          <button class="rl-ai-close" type="button" aria-label="Close">×</button>
        </header>
        <div class="rl-ai-suggestions">
          <button class="rl-ai-suggestion" type="button">What needs attention first?</button>
          <button class="rl-ai-suggestion" type="button">Find resource gaps</button>
          <button class="rl-ai-suggestion" type="button">Draft an agency alert</button>
        </div>
        <div class="rl-ai-messages"></div>
        <div class="rl-ai-input">
          <textarea placeholder="Ask about incidents, teams, resources, priorities…"></textarea>
          <div class="rl-ai-actions"><button class="rl-ai-clear" type="button">Clear</button><button class="rl-ai-send" type="button">Send</button></div>
          <div class="rl-ai-status">AI suggestions are advisory. A human coordinator must approve real-world actions.</div>
        </div>
      </section>`;

    // Put the coordinator inside the current Multi-Agency view rather than fixed to the whole app.
    const app = document.querySelector('#app');
    const target = app && (app.querySelector('.view-content, .page-content, .content-area, .main-content') || app);
    ...
    if (!state.messages.length) state.messages.push({ role:'assistant', content:'Hello. I\'m the ReliefLink AI Coordinator. I can analyze the current incidents, teams, resources and agency feed, help prioritize work, and draft messages. Real-world actions require human confirmation.' });
    renderMessages(ui.messages);
  ...
  function syncVisibility() {
    if (isFeedView()) mount(); else unmount();
  }
