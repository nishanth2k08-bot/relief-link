/* ReliefLink AI Coordinator — secure chat UI for the Multi-Agency feed. */
(function () {
  'use strict';
  ...
  function getAppState() {
    try {
      return JSON.parse(localStorage.getItem('relieflink_state_v1') || '{}');
    } catch (_) {
      return {};
    }
  }
  ...
  function addStyles() {
    ...
    style.id = STYLE_ID;
    style.textContent = `
      #${PANEL_ID} { position:fixed; right:24px; bottom:24px; z-index:99999; font-family:Inter,system-ui,sans-serif; }
      #${PANEL_ID} .rl-ai-launch { border:0; border-radius:999px; padding:13px 18px; background:#2563eb; color:#fff; font-weight:800; cursor:pointer; box-shadow:0 10px 30px rgba(0,0,0,.28); display:flex; align-items:center; gap:9px; }
      #${PANEL_ID} .rl-ai-window { display:none; width:min(420px,calc(100vw - 32px)); height:min(620px,calc(100vh - 110px)); background:#0f172a; color:#e5e7eb; border:1px solid rgba(148,163,184,.25); border-radius:18px; overflow:hidden; box-shadow:0 25px 70px rgba(0,0,0,.45); }
  ...
  function mount() {
    ...
    state.messages.push({ role:'assistant', content:'Hello. I\'m the ReliefLink AI Coordinator. I can analyze the current incidents, teams and resources, help prioritize work, and draft messages for the agencies. I will not claim that a real-world action happened unless the system confirms it.' });
    renderMessages(ui.messages);
